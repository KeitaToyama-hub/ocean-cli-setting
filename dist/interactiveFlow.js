...
const validateIPFS = (input) => /^Qm[1-9A-HJ-NP-Za-km-z]{44}$/.test(input) || 'Invalid IPFS hash format.';
const validateArweave = (input) => /^[a-zA-Z0-9_-]{43,}$/.test(input) || 'Invalid Arweave transaction ID format.';
const validateURL = (input) => /^(https?:\/\/)[^\s/$.?#].[^\s]*$/.test(input) || 'Invalid URL format.';
export async function interactiveFlow(providerUrl) {
    try {
        console.clear();
        console.log(chalk.blue(figlet.textSync('Ocean CLI', { horizontalLayout: 'full' })));
        ...
        // Storage type prompt
        const { storageType } = await prompt({
            type: 'select',
            name: 'storageType',
        ...
        const { assetLocation } = await prompt([
            {
                type: 'input',
                name: 'assetLocation',
