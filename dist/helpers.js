...
import { AccesslistFactory, Nft, NftFactory, ProviderInstance, ZERO_ADDRESS, approveWei, orderAsset, getEventFromTx, LoggerInstance, createAsset } from "@oceanprotocol/lib";
import { homedir } from "os";
const ERC20Template = readFileSync('./node_modules/@oceanprotocol/contracts/artifacts/contracts/templates/ERC20Template.sol/ERC20Template.json', 'utf8');
export async function downloadFile(url, downloadPath, index) {
...
export async function calculateActiveTemplateIndex(owner, nftContractAddress, // addresses.ERC721Factory,
template) {
    // is an ID number?
    ...
    for (let i = 1; i <= currentTokenCount; i++) {
        ...
        const erc20Template = new ethers.Contract(tokenTemplate.templateAddress, ERC20Template.abi, owner);
        // check for ID
        if (isTemplateID) {
        ...
        else if (tokenTemplate.isActive &&
...
export function getSignerAccordingSdk(signer, config) {
    return config && 'sdk' in config && config.sdk === 'oasis'
        ? sapphire.wrap(signer)
        : signer;
}
export async function createAssetUtil(name, symbol, owner, assetUrl, ddo, oceanNodeUrl, config, aquariusInstance, encryptDDO = true, templateIDorAddress = 1, // If string, it's template address , otherwise, it's templateId,
providerFeeToken = ZERO_ADDRESS, accessListFactory, allowAccessList, denyAccessList) {
    ...
    if (config.sdk === 'oasis') {
        ...
        // Create Allow List
        await accessListFactoryObj.deployAccessListContract('AllowList', 'ALLOW', ['https://oceanprotocol.com/nft/'], false, await owner.getAddress(), [await owner.getAddress(), ZERO_ADDRESS]);
...
export async function updateAssetMetadata(owner, updatedDdo, oceanNodeUrl, aquariusInstance, encryptDDO = true) {
    ...
    if (encryptDDO) {
        const providerResponse = await ProviderInstance.encrypt(updatedDdo, updatedDdo.chainId, oceanNodeUrl);
    ...
    const updateDdoTX = await nft.setMetadata(updatedDdo.nftAddress, await owner.getAddress(), 0, oceanNodeUrl, "", toBeHex(flags), metadata, validateResult.hash);
    return updateDdoTX;
}
export async function handleComputeOrder(order, asset, payerAccount, consumerAddress, serviceIndex, datatoken, config, providerFees, oceanNodeUrl, consumeMarkerFee) {
    ...
    */
    ...
    if (hasProviderFees && Number(order.providerFee.providerFeeAmount) > 0) {
        await approveWei(payerAccount, config, await payerAccount.getAddress(), order.providerFee.providerFeeToken, asset.services[0].datatokenAddress, order.providerFee.providerFeeAmount);
    }
    if (order.validOrder) {
        if (!order.providerFee)
            return order.validOrder;
        const tx = await datatoken.reuseOrder(asset.services[0].datatokenAddress, order.validOrder, order.providerFee);
    ...
    const txStartOrder = await orderAsset(asset, payerAccount, config, datatoken, oceanNodeUrl, consumerAddress, consumeMarkerFee, providerFees);
    if (!txStartOrder)
        return;
...
export async function isOrderable(asset, serviceId, algorithm, algorithmDDO) {
        ...
        if (algorithm.documentId) {
            ...
            if (algoService && algoService.type === "compute") {
                if (algoService.serviceEndpoint !== datasetService.serviceEndpoint) {
                    LoggerInstance.error("ERROR: Both assets with compute service are not served by the same provider");
                    return false;
                }
            }
        }
...
export function getIndexingWaitSettings() {
...
export function fixAndParseProviderFees(rawString) {
...
export async function getConfigByChainId(chainId) {
    ...
    const chainConfig = Object.values(data).find((network) => network.chainId === chainId);
    if (!chainConfig) {
